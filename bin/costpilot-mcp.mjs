...
const apiUrl = process.env.COSTPILOT_API_URL?.trim();
...
async function apiFetch(requestPath, init = {}) {
  if (!apiUrl) {
    throw new Error("COSTPILOT_API_URL is required.");
  }
...
async function loginEmployee() {
  ...
  if (!apiUrl) {
    throw new Error("Set COSTPILOT_API_URL before using CostPilot MCP.");
  }
